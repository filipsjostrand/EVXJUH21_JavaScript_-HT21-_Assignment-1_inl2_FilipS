...
const testElement = document.getElementById("answer-one");
...
const testElement2 = document.getElementById("answer-two");

function taskTwo() {
  numbers.push(Math.floor(Math.random() * 100));
  testElement2.innerHTML = "[" + numbers + "]";
}
...
const newElement5 = document.getElementById("answer-five");
...
function taskFive() {
  ...
  newElement5.innerHTML = "<p>" + "<b>Ätligt: </b>" + eatable + "<br>" + "<b>Skräp: </b>" + trash; + "</p>"
};
...
const newElement6 = document.getElementById("answer-six");
...
function taskSix() {
  ...
  newElement6.innerHTML = `<p> <b>Över 30: </b> ${overThirty} <br> <b>Under 30: </b> ${underThirty} <br> <b>Gift: </b> ${married} <br> <b>Ogift: </b> ${notMarried} <br> </p>`
};
...
*/
const newElement = document.getElementById("answer-seven");
...
function taskSeven() {
  const addMeSomeLineBreaksDivided = addMeSomeLineBreaks.replace(/,/gi, "<br>");
  newElement.innerHTML = addMeSomeLineBreaksDivided;
}
...
const elementToHide = document.getElementById("card-eight");

function taskEight() {
  setTimeout(() => 
    {elementToHide.style.display = "none"}, 0);
  setTimeout(() => 
    {elementToHide.style.display = "block"}, 3000);
};
...
const newElements = document.getElementsByClassName("answer-container");
...
function taskNine() {
  if (hour >= 17 && hour <= 23) {
      for (i = 0; i <= 9; i++) {
        newElements[i].style.backgroundColor = "blue";
    }
  } else {
      for (j = 0; j <= 9; j++) {
        newElements[j].style.backgroundColor = "red";
      }
  }
};
